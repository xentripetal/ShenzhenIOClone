...
namespace Zachclone.Instructions {
    public class TeqInstruction : Instruction {
        private RIArg firstArg;
        private RIArg secondArg;

        public TeqInstruction(Chip chip, string[] args, int relativePos, int actualPos, TestPrefix testPrefix) : base(chip, relativePos, actualPos, testPrefix) {
            if (args.Length != 2) {
